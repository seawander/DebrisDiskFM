...
def generateMcfostTemplate(n_zone = 1, n_species = [1], n_star = 1):
    ...
    paramfile_dict['#Wavelength'] = collections.OrderedDict([
         ...
         ('row2', collections.OrderedDict([
                   ('wavelength file', 'hd35841_irs+mips+scuba_n30.lambda')])),
         ('row3', collections.OrderedDict([
                   ('separation of different contributions?', 'T'),
                   ('stokes parameters?', 'T')]))])
...
n_species = [1]
n_star = 1
sample_para_dict = generateMcfostTemplate(n_zone = n_zones, n_species = n_species, n_star = n_star)

def print0_mcfost_version(sample_para_dict = sample_para_dict):
    print(sample_para_dict['mcfost version'], '\t', 'mcfost version')
    print()
    
def print1_Number_of_photon_packages(sample_para_dict = sample_para_dict):
...
def print2_Wavelength(sample_para_dict = sample_para_dict):
...
def print3_Grid_geometry_and_size(sample_para_dict = sample_para_dict):
...
def print4_Maps(sample_para_dict = sample_para_dict):
    block_name = '#Maps'
    print(block_name)
    for row_name in sample_para_dict[block_name]:
        if type(sample_para_dict[block_name][row_name]) != type(collections.OrderedDict()):
...
def print5_Scattering_method(sample_para_dict = sample_para_dict):
...
def print6_Symmetries(sample_para_dict = sample_para_dict):
...
def print7_Disk_physics(sample_para_dict = sample_para_dict):
...
def print8_Number_of_zones(sample_para_dict = sample_para_dict):
...
def print9_Density_structure(sample_para_dict = sample_para_dict, n_zones = n_zones):
...
def print10_Grain_properties(sample_para_dict = sample_para_dict, n_zones = n_zones, n_spieces = n_spieces):
...
def print11_Molecular_RT_settings(sample_para_dict = sample_para_dict):
...
def print12_Star_properties(sample_para_dict = sample_para_dict, n_star = n_star):
    block_name = '#Star properties'
    print(block_name)
    print(sample_para_dict[block_name]['Number of stars'], '\tNumber of stars')
    for star_id in range(n_star):
...
def print_file():
    print0_mcfost_version()
    print1_Number_of_photon_packages()
    print2_Wavelength()
    print3_Grid_geometry_and_size()
    print4_Maps()
    print5_Scattering_method()
    print6_Symmetries()
    print7_Disk_physics()
    print8_Number_of_zones()
    print9_Density_structure()
    print10_Grain_properties()
    print11_Molecular_RT_settings()
    print12_Star_properties()
    
import sys
sys.stdout = open("./test.para", "w+")
print_file()
